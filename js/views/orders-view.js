...
function renderFloorPlan() {
    ...
    allTables.forEach(table => {
        const activeOrder = state.activeOrders.find(order => {
            ...
            return customer === table.name || (order.deliveryType === 'mesa' && customer.startsWith(table.name + ' - '));
        });
...
function renderActiveOrders() {
    ...
    pendingOrders.forEach((order, index) => {
        ...
        (order.items || []).forEach(item => {
            ...
            const key = `${item.name}|${item.type}|${item.serviceType}|${details}|${item.paid ? 'paid' : 'unpaid'}|${item.isAdditional ? 'additional' : 'original'}`;
            if (!groups[key]) {
                groups[key] = { name: item.name, serviceType: item.serviceType, details, qty: 0, total: 0, paid: item.paid, isAdditional: item.isAdditional };
                orderKeys.push(key);
            }
        ...
        card.innerHTML = `
            ...
            <div class="order-card-footer">
                ...
                <div class="order-card-total-row"><span>Total a cobrar:</span><span>${formatCurrency(order.total)}</span></div>
                <div class="order-card-actions"><button class="btn btn-outline btn-sm" onclick="openAppendItemsModal('${order.id}')"><i class="fa-solid fa-cart-plus"></i> Agregar</button>${(state.business?.waiter_ticket_enabled === '1' || state.business?.waiter_ticket_enabled === true) && (order.items || []).some(item => item.serviceType === 'servirse') ? `<button class="btn btn-outline btn-sm" onclick="PrintJobs.printWaiterTicket(state.activeOrders.find(item => item.id === '${order.id}'))"><i class="fa-solid fa-user-tie"></i> Ticket mesero</button>` : ''}${order.paid || (order.items || []).every(item => item.paid) ? `<button class="btn btn-success btn-sm" onclick="closePaidOrder('${order.id}')"><i class="fa-solid fa-check-double"></i> Cerrar</button>` : ''}${!order.paid ? `<button class="btn btn-outline-danger btn-sm" onclick="cancelActiveOrder('${order.id}')"><i class="fa-solid fa-ban"></i> Cancelar</button>` : '<span class="text-muted" style="font-size:10px;text-align:center;max-width:100px;">Cobrado: usar Devoluciones</span>'}</div>
            </div>
        `;
